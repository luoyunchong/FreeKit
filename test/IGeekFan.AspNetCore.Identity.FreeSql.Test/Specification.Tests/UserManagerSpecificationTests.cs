    ...
    where TKey : IEquatable<TKey>
{
    ...
    public async Task CanCreateUserLoginAndAddPassword()
    {
        ...
        Assert.NotNull(logins);
        Assert.Single(logins);
        MyAssert.Equal(user, await manager.FindByLoginAsync(login.LoginProvider, login.ProviderKey));
        Assert.True(await manager.CheckPasswordAsync(user, "password"));
    }
    ...
    public async Task CanFindByEmail()
    {
        ...
        Assert.Equal(JsonConvert.SerializeObject(user), JsonConvert.SerializeObject(fetch));
    }
